...
driver.get("https://www.studiomlighting.com/indoor-lighting/collections/allwall" )
...
studio_wall_collection_urls =[]
studio_wall_collections = driver.find_elements(By.XPATH, ".//div[@class='pdf-thumb-box divProduct']/a")

for x in studio_wall_collections:
    product_url = x.get_attribute('href')
    studio_wall_collection_urls.append(product_url)

for product in studio_wall_collection_urls:
    driver.get(product)
    studio_product_urls = []
    studio_wall_collections = driver.find_elements(By.XPATH, ".//div[@class='pdf-thumb-box divProduct']/a")

    for foo in studio_wall_collections:
...
studio_data = pd.DataFrame(studio).to_csv("wall_csv.csv", index=False)
