...
driver.get('https://visionairelighting.com/visionaire-products/wall-and-ceiling/')

wall_data = {"name": [], "description": [], "link_url": [], "image_url": [], "manufacturer_id": []}
...
for n in product_names:
  foo = n.text
  wall_data["name"].append(foo)
  
for p in products:
  links = p.get_attribute('href')
  product_links.append(links)
  wall_data["link_url"].append(links)
...
for url in product_links:
  ...
  wall_data["image_url"].append(all_pics)
  
  try:
    description = driver.find_element(By.XPATH, "//*[@class='et_pb_text_inner']/p[2]").text
    wall_data["description"].append(description)
  except:
  ...
  manufacturer_id = 159
  wall_data["manufacturer_id"].append(manufacturer_id)
    
final_wall_data = pd.DataFrame(wall_data)
final_wall_data.to_csv("wall_csv.csv", index=False)
